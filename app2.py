...
st.info(
    "Uwaga: Dla efektywności aplikacja przetwarza maksymalnie pierwsze 30 i ostatnie 25 stron PDF. "
...
def generate_toc_with_gpt4o(pdf_text):
    ...
    prompt = """ 
    Instrukcja: Jesteś asystentem AI, który pomaga użytkownikom generować kod HTML dla spisu treści na podstawie przesłanych plików PDF lub DOCX. Twoim zadaniem jest przetworzenie dokumentu, wykrycie struktury spisu treści i wygenerowanie odpowiednio sformatowanej tabeli HTML. Przeanalizuj dokument pod kątem wielopoziomowej struktury i dokładnie rozpoznaj wszystkie poziomy hierarchii. Następnie wygeneruj tabelę w formacie HTML, tak aby była gotowa do skopiowania i implementacji. Nie zajmuj się frontendem. Pamiętaj żeby wygenerować cały spis treści a nie tylko kawałek. Nie dodawaj nic od siebie, korzystaj tylko z danych zawartych w pliku. Opieraj się tylko na spisie treści dostępnym w pliku. Zawsze generuj kompletną tabelę HTML w jednym bloku kodu. Zachowaj pełną strukturę spisu treści, w tym wszystkie poziomy (rozdziały, podrozdziały). Upewnij się, że żaden element nie zostanie pominięty. Poszczególne kroki:
    ...
    3. Generuj kod HTML w tabeli <table>, stosując poniższy format. Na podstawie stylu spisu treści, dostosuj wcięcia w kodzie. Tylko główne rozdziały umieść w znaczniki <strong></strong> oraz dodaj puste wiersze <tr><td> </td><td> </td></tr> po każdym z nich.
    <table>
...
# Nowa funkcja do dzielenia tekstu na mniejsze fragmenty
...
if uploaded_file:
    with st.spinner("📖 Przetwarzanie pliku..."):
        ...
        if extracted_text.strip():
            ...
            text_chunks = chunk_text(extracted_text, max_length=3000)
            toc_fragments = []
            for chunk in text_chunks:
                toc_fragment = generate_toc_with_gpt4o(chunk)
                toc_fragments.append(toc_fragment)

            # Łączymy fragmenty w jeden wynik
            full_toc_html = "\n".join(toc_fragments)

            st.subheader("📑 Wygenerowany Spis Treści")
            st.markdown(full_toc_html, unsafe_allow_html=True)
        else:
